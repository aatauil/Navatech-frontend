...
function Products(props) {
    ...
    if(showFilter){
        filter =
        <div className="top-0 bg-white w-full h-screen fixed">
             <button className=" py-4 bg-gray w-full" onClick={() => setShowFilter(false)}> <FontAwesomeIcon icon={faTimes}/> Close Filter</button>
             <div className="h-auto w-full mr-6 p-6 ">
                    <div className="">
                        <label className="block uppercase tracking-wide text-xs font-bold mb-2 mt-6" htmlFor="grid-state">
                            Model
                        </label>
                        ...
                        <button onClick={() => setShowFilter(false)} className="mt-6 px-6 py-2 bg-black text-white rounded shadow text-sm">Update</button>
                    </div>
